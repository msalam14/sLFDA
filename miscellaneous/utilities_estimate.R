#' This function generates results for every simulation performed in Alam and Staicu (20xx)
#' 
...
sim_iter_slfda<-function(iter,n,nTEST,base_seed=100){
  ...
  slfda<-skewedFDA(funDATA=trainY,funARG=ss,obsTIME=Tij,ETGrid=tp,DOP=1,KernelF=depan,CV=TRUE,Hgrid=seq(0.02,0.2,0.02),CVThresh = 0.05,PenaltyF=Qpenalty,plfGT=Tg,
                   ...
                   Cov2nbasis=c(20,20),PVE=c(0.95,0.90),PSid=c(NTest,FTest),PredGS=NULL,PredGT=c(NTij,FTij),
                   Prediction=TRUE)
  ...
  Mean<-slfda$Mean; Scale<-slfda$Scale; Alpha<-slfda$Alpha; h<-slfda$h
  ...
  pred_slfda<-predict_slfda(slfda,outSAMPLE = TRUE,nfunDATA = Vdata,nfunARG = ss,
  ...
  Mean0<-slfda0$Mean; Scale0<-slfda0$Scale; Alpha0<-slfda0$Alpha; h0<-slfda0$h
  
  pred_slfda0<-predict_slfda(slfda0,outSAMPLE = TRUE,nfunDATA = Vdata,nfunARG = ss,
                             nobsTIME = OSOTij,identical.ARG = TRUE,PredGT = OSPTij)
  ...
  cobsQ<-cbind(do.call(rbind,lapply(qprb,function(u){
    ...
    gfit<-gam(Qy~te(Space,Time,k=c(20,20),bs=c("ps","ps"),m=c(2,2)),data = qdat,method = "REML")
  ...
  fitLFDA<- fpcaLFDA(Y = do.call(rbind,trainY), subject.index = do.call(c,lapply(1:n, function(i){rep(i,mi[i])})),
                     ...
                     funcArg = ss,numTEvalPoints = 51,fbps.knots = 20, fbps.p = 3, fbps.m = 2,
                     LongiModel.method='fpca.sc',
                     mFPCA.pve = 0.95, mFPCA.knots = 20, mFPCA.p = 3, mFPCA.m = 2, 
                     sFPCA.pve = 0.90, sFPCA.nbasis = 20)   
  ...
  LPREV<-PWPRED(lfdaOBJ = fitLFDA,TimeOBJ = do.call(c,Tij),gridT = c(NTij,FTij),gTID=c(NTest,FTest))  
  ...
  osLFDA<-OutSamPWPRED(fitLFDA,TimeOBJ=do.call(c,Tij),nfunDATA=Vdata,nfunARG=ss,nobsTIME=OSOTij,
...
for(i in 1:3){
  ...
  for(l in 1:3){
    ...
    repeat{
      ...
      write.table(plf_dat,file = paste(dirP,"PLF_",fnam[i],"_",n,".txt",sep=""),
      ...
      write.table(ipe_dat,file = paste(dirP,"IPE_",fnam[i],"_",n,".txt",sep=""),
                  append = TRUE,col.names = FALSE,row.names = FALSE)
      ...
      write.table(lfdaQ,file = paste(dirP,"QLFDA_",fnam[i],"_",n,".txt",sep=""),
                  append = TRUE,col.names = FALSE,row.names = FALSE)
      ...
      write.table(cobsQ,file = paste(dirP,"QCOBS_",fnam[i],"_",n,".txt",sep=""),
                  append = TRUE,col.names = FALSE,row.names = FALSE)
